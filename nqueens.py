...
def main():
    while True:
        ...
        try:
            ...
            while True:
                row = (input(f"Enter the row coordinate (0 to {n-1}): "))
                if(row == "-0") :
                    print(f"Invalid Input. Enter a positive integer from (0 to {n-1}) : ")
                else :
                    row = int(row)
                    break
            while True:
                col = (input(f"Enter the row coordinate (0 to {n-1}): "))
                if(col == "-0") :
                    print(f"Invalid Input. Enter a positive integer from (0 to {n-1}) : ")
                else :
                    col = int(col)
